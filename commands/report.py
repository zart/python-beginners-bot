...
def my_report(message):
    ...
    if not r.get(message.reply_to_message.message_id):
        ...
        response = bot.send_message(
            chat_id=message.chat.id, text="Мы примем все необходимые меры, спасибо."
        )
        Timer(
            30,
            bot.delete_message,
            kwargs={"chat_id": response.chat.id, "message_id": response.message_id},
        ).start()
...
def ro_giver(message, r):
    ...
    if message.from_user.id in config.admin_ids:
        logger.info(
            "Admin {} is flooding with !report. Doing nothing".format(
    ...
    if user_obj.ro_level < 4:
        ...
        logger.info(
            "User {} has been banned for flooding with !report".format(
                get_user(message.from_user)
            )
        )
    ...
    for admin_id in config.admin_ids:
        try:
            ...
            if str(e.result) == config.unreachable_exc:
                continue
    logger.info(
        "Message {} has been successfully reported".format(
