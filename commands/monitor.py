...
def scan_contents(message):
    ...
    if message.forward_from_chat:
        if (
            ...
            and message.forward_from_chat.id not in config.whitelist_channels
        ):
            punisher(message)
            return
...
def forbidden_entities(message):
    ...
    for entity in message.entities:
        entity_text = msg_enc[
            2 * entity.offset : 2 * (entity.offset + entity.length)
        ].decode("utf-16le")
        if entity.type == "url":
            if "/joinchat/" in entity_text:
                logger.info(
                    "Message {} has a channel link in an URL entity".format(
                        message.message_id
                    )
                )
                return True
            ...
            if "/joinchat/" in entity.url:
                logger.info(
                    "Message {} has a channel link in a text-link entity".format(
                        message.message_id
                    )
                )
                return True
            ...
            if bot.get_chat(entity_text).type == "channel":
                logger.info(
                    "Message {} has a channel link in a mention entity".format(
                        message.message_id
                    )
                )
                return True
...
def punisher(message):
    ...
    if r.get("spammer_{}".format(message.from_user.id)):
        ...
        logger.info(
            "Message {} has been deleted without alerting the admins due to flooding".format(
                message.message_id
            )
        )
        return
    ...
    bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
    logger.info(
        "Message {} has been successfully auto-reported".format(message.message_id)
    )
