...
def validate_document(message):
    if message.document:
        ...
        return (
            os.path.splitext(file_name)[-1] in config.EXTENSIONS
            and file_size <= config.MAX_FILE_SIZE
        )
...
def validate_command(
    message,
    check_isprivate=False,
    check_isinchat=False,
    check_isreply=False,
    check_isadmin=False,
):
    ...
    if check_isprivate and message.chat.type != "private":
        logger.info(
            "User {0} called {1} not in a private chat. Aborting".format(
                get_user(message.from_user), message.text.split(" ")[0]
            )
        )
        return False
    ...
    if check_isreply and getattr(message, "reply_to_message") is None:
        logger.info(
            "User {0} called {1} the wrong way".format(
                get_user(message.from_user), message.text.split(" ")[0]
            )
        )
        return False
    ...
    if check_isadmin and message.from_user.id not in config.admin_ids:
        logger.info(
            "User {0} tried to call {1}. Aborting".format(
                get_user(message.from_user), message.text.split(" ")[0]
            )
        )
        return False
...
def get_admins(chat):
    ...
    return [
        admin.user.id
        for admin in bot.get_chat_administrators(chat)
